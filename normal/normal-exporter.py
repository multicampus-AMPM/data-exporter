...
class NormalExporter(object):
    ...
    def collect(self):
        ...
        if self.current_row > -1:
            ...
            self.add_metrics(self.data.iloc[self.current_row], self.current_row, 'sda', metric_pretty, metric_current)
            sdb_current_row = self.length - self.current_row
            self.add_metrics(self.data.iloc[sdb_current_row], sdb_current_row, 'sdb', metric_pretty, metric_current)
        self.current_row += 1
        yield metric_current
        yield metric_pretty
    
    def add_metrics(self, row, idx, drive, metric_pretty, metric_current):
        for index, value in row.items():
            ...
            labels = [self.instance, drive, attr_name, str(idx)]
...
if __name__ == '__main__':
    ...
    exporter.registry.register(NormalExporter(file_path, app.logger))
